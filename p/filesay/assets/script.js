                ...
                .then(helix => {
                    ...
                    new WebSocket("wss://irc-ws.chat.twitch.tv/").onopen = async function () {
                        ...
                        new Promise(async (resolve) => {
                            ...
                            resolve(`<div class="text"><img src="https://cdn.frankerfacez.com/emote/250522/1" alt="FeelsOkayMan" width="30">Successfully sending ${Number(text.split("\n").length)}. messages in ${String(channel.replace(/</g, "&#60;").replace(/>/g, "&#62;"))}!</div>`);
                        }).then(async (value) => {
                            this.close();
                            document.querySelector('#button').disabled = true
                            setTimeout(() => {
