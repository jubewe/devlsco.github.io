...
async function send() {
    ...
    await fetch("https://id.twitch.tv/oauth2/validate", options)
        ...
        .then(async result => {
            ...
            if (!sizeRegEx.test(String(size))) return response.innerHTML = `<div class="text"><img src="https://cdn.7tv.app/emote/63071bb9464de28875c52531/1x" alt="FeelsDankMan" width="30">Error: size must match pattern "${sizeRegEx}".</div>`;
            if (Number(size > 25)) return response.innerHTML = `<div class="text"><img src="https://cdn.7tv.app/emote/63071bb9464de28875c52531/1x" alt="FeelsDankMan" width="30">Error: size maximum. 25</div>`;
                    ...
                    new WebSocket("wss://irc-ws.chat.twitch.tv/").onopen = async function () {
                        ...
                        new Promise(async (resolve) => {
                            ...
                            for (let i = 1; i <= size; i++) {
                                await this.send(`PRIVMSG #${channel} :${(text + " ").repeat(i)}`);
                                await sleep(100)
                            }
                            ...
                            resolve(`<div class="text"><img src="https://cdn.frankerfacez.com/emote/250522/1" alt="FeelsOkayMan" width="30">Successfully sending ${Number(size.replace(/</g, "&#60;").replace(/>/g, "&#62;"))}. Pyramid length in ${String(channel.replace(/</g, "&#60;").replace(/>/g, "&#62;"))}!</div>`);
                        }).then(async (value) => {
                            this.close();
                            document.querySelector('#button').disabled = true
                            setTimeout(() => {
